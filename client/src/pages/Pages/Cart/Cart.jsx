...
const Cart = ({ user }) => {
  ...
  const payViaUpi = async () => {
    try {
      ...
      const isbns = Array.isArray(data) ? data.map((b) => b.ISBN).filter(Boolean) : [];
      if (isbns.length === 0) return;
      const payload = {
        username: user.username,
        isbns,
        shippingAddress: "",
        payerName: user.name || user.username,
      };
      const { data: resp } = await axios.post(
        "http://localhost:5000/payments/upi/initiate",
        payload
      );
      if (resp?.upiLink) {
                  ...
                  <div style={{ padding: "2rem" }}>
                    <div style={{ display: "flex" }}>
                      ...
                      <div
                        ...
                        onClick={payViaUpi}
                      >
                        {initiatingPayment ? "Starting GPay..." : "Pay via UPI (GPay)"}
                      </div>
                    </div>
                    {data.map((d, i) => (
                      <div
